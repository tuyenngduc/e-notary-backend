...
public class UserService {
    ...
    @Transactional
    public User toggleUserStatus(UUID targetUserId, UUID adminUserId) {
        User admin = userRepository.findById(adminUserId)
                .orElseThrow(() -> new AppException("Không tìm thấy quản trị viên", HttpStatus.UNAUTHORIZED));
        ...
        if (target.getUserId().equals(adminUserId)) {
            throw new AppException("Không thể khóa/mở khóa chính tài khoản quản trị đang đăng nhập", HttpStatus.BAD_REQUEST);
        }
        ...
        if (normalizedDefaultAdmin != null
                && !normalizedDefaultAdmin.isBlank()
                && normalizedDefaultAdmin.equals(normalizeEmail(target.getEmail()))) {
            throw new AppException("Không thể khóa tài khoản admin mặc định", HttpStatus.BAD_REQUEST);
        }
        ...
        String action = target.getIsActive() ? "USER_UNBLOCKED" : "USER_BLOCKED";
        auditLogService.logAction(admin, action, "users", target.getUserId());
